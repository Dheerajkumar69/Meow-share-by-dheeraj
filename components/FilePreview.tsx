  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-sm text-blue-600">{formatSize(file.size)}</span>
      </div>
      ...
      <div className="glassmorphism overflow-hidden rounded-xl">
        ...
        {fileType === 'audio' && preview && (
          <div className="py-8 px-4 flex flex-col items-center">
            <div className="w-24 h-24 rounded-full bg-gradient-to-r from-blue-400 to-blue-600 flex items-center justify-center mb-4 animate-pulse-slow">
        ...
        {fileType === 'other' && (
          <div className="p-8 flex flex-col items-center">
            ...
            <p className="text-center text-blue-800 font-medium">{file.name}</p>
          </div>
        )}
      </div>
      ...
      <div className="mt-4 p-4 glassmorphism rounded-xl">
        <div className="flex flex-col gap-2">
          <div className="flex justify-between">
            ...
            <span className="text-blue-900 truncate max-w-xs">{file.name}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-blue-600">Type:</span>
            <span className="text-blue-900">{file.type || 'Unknown'}</span>
          </div>
          <div className="flex justify-between">
...
function FileIcon({ type }: { type: string }) {
  ...
  if (fileType === 'application' && ['zip', 'x-zip-compressed', 'x-rar-compressed'].includes(fileExtension)) {
    return (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-full h-full text-amber-500">
